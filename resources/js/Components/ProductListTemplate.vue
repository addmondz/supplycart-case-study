<template>
    <div
        class="pro-detail w-full flex flex-col justify-center order-last lg:order-none max-lg:max-w-[608px] max-lg:mx-auto border p-10 rounded-xl bg-white overflow-hidden border-white hover:border-violet-600 transition-all duration-500">
        <p class="font-medium text-lg text-indigo-600 mb-4">
            <span class="font-extrabold uppercase">{{ product.brand.name }}</span> / {{ product.category.name }}
        </p>
            ...
            <h6 class="font-manrope font-semibold text-2xl leading-9 text-gray-900 pr-5 mr-5">
                <span v-if="$page.props.auth.user.membership_type_id != 1">
                    <span v-if="storedUserData">
                        <span class="line-through text-base text-gray-400">${{ formatPrice(product.price) }}</span>
...
import { formatPrice, apiBaseUrl } from '@/Helpers/helpers.js';

const productCount = ref(1);

const updateCount = (delta) => {
    productCount.value += delta;
    if (productCount.value < 1) productCount.value = 1; // Ensure minimum count is 1
};
...
const addToCart = async () => {
    try {
        ...
        productCount.value = 1; // Reset count after adding to cart
    } catch (error) {
        Swal.fire({
            title: 'Error!',
...
const itemPrice = computed(() => {
    ...
    const discount = storedUserData?.membership_type?.discount ?? 0;
